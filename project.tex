...
\end{table}
\newpage
\section*{Adiabatic Quantum Algorithm}
...
\begin{itemize}
	...
	$C^n_s(a)$ is the total clique of size s found in the graph $a$
...
\begin{algorithm}[H]
	...
	\Begin{
		...
		\For{graph $= 1$ to $N$ }{\If{$a(n,graph) $ has $clique$ or $independent\_set$}{$h \gets$ total number(cliques)+ total number($independent\_set$)}} 
		\KwRet $h$\;
	}
