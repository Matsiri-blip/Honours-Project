...
\includepdf[pages={4,3,2,1}]{binary_homomorphic_groups.pdf} which concluded the weeks progress, so we basically found a way to draw our graphs in a way which we can identify each individual graph using Binary counting. This is the efficient way so far that we can understand.
\end{document}